...
CBlockTemplate* CreateNewBlockInner(const CScript& scriptPubKeyIn, bool fAddProofOfStake, bool& fPoSCancel, CWallet* pwallet)
{
    ...
    if (fAddProofOfStake)  // attemp to find a coinstake
    {
        ...
        if (fPoSCancel)
            return NULL; // there is no point to continue if we failed to create coinstake
    }
    else
        pblock->nBits = GetNextWorkRequired(pindexPrev, pblock, false);
...

    {
        ...
        CValidationState state;
        if (!TestBlockValidity(state, *pblock, pindexPrev, false, false, false)) // we do not check block signature here, since we did not sign it yet
...
{
    LogPrintf("LitecoinMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread(fProofOfStake? "litecoin-stake-minter" : "litecoin-miner");
